...
// Project routes: public browsing + admin-only CRUD
...
// Public: list all projects with their owners
...
// Update existing project (must be owner)
router.put('/:id', async (req, res) => {
  ...
  const fields = ['title','description','github_link','demo_link','image_url']
    .map(f => req.body[f]);
  const q = `
    UPDATE projects 
    SET title=$1, description=$2, github_link=$3, demo_link=$4, image_url=$5, updated_at=CURRENT_TIMESTAMP 
    WHERE id=$6 
    RETURNING *`;
